...
namespace TankLike.Combat.SkillTree
{
    ...
    public class SkillTreeUpgradesController : MonoBehaviour
    {
        ...
        public List<SkillUpgrade> GetUpgrades(UpgradeTypes type, int playerIndex)
        {
            ...
            List<SkillUpgrade> upgrades = list.Upgrades.FindAll(u => u.IsAvailable && u is not SkillsConditionalContainer);

            List<SkillsConditionalContainer> conditionalUpgrades = list.Upgrades
                .OfType<SkillsConditionalContainer>()
                .ToList();

            for (int i = 0; i < conditionalUpgrades.Count; i++)
            {
                var availableUpgrades = conditionalUpgrades[i].GetAvailableUpgrades();

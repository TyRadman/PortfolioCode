...
namespace TankLike.Combat.SkillTree
{
    ...
    public class SkillTreeRandomSkillMenuController : Navigatable, IInput
    {
        ...
        private void SelectAction(InputAction.CallbackContext obj)
        {
            ...
            activeCell.SetName(_activeCell.GetName());
            activeCell.SetDescription(_activeCell.GetDescription(_player));
            activeCell.SetIcon(_activeCell.OnUnlockedUpgrade.GetIconSprite());

            //Debug.Log($"Activating cell with id {_activeCell.GetInstanceID()}");
            //_player.Upgrades.Upgrade(_activeCell.OnUnlockedUpgrade.GetUpgrade());
        ...
        {
            _UIDisplayer.UpdateDescription(_activeCell.GetDescription(_player));
        }
        #endregion
        ...
        {
            //Debug.Log($"Setting first skill to cell {_firstCell.GetInstanceID()} and cell {_secondCell.GetInstanceID()}\n" +
            //    $"Skills are {firstSkill.GetInstanceID()} and {secondCell.GetInstanceID()}");

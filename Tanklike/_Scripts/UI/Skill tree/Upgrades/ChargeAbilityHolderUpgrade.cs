...
namespace TankLike.Combat.SkillTree.Upgrades
{
    ...
    public class ChargeAbilityHolderUpgrade : SkillUpgrade
    {
        ...
        public override void SetUpgradeProperties(PlayerComponents player)
        {
            ...
            if (ChargeDuration != 0)
            {
                float previousHoldDuration = player.ChargeAttack.GetAbilityHolder().HoldDownDuration;
            ...
            {
                Vector2 previousRange = player.ChargeAttack.GetAbilityHolder().PerfectChargeRange;

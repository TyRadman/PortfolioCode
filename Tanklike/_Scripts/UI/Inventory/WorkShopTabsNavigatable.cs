...
namespace TankLike.UI.Workshop
{
    ...
    public class WorkShopTabsNavigatable : Navigatable, IInput
    {
        ...
        public override void SetUpSignifiers()
        {
            ...
            int switchTabLeftIconIndex = GameManager.Instance.InputManager.GetButtonBindingIconIndex(c.UI.Tab_Left.name, PlayerIndex);
            _switchTabLeftText.text = Helper.GetInputIcon(switchTabLeftIconIndex);

            int switchTabRightIconIndex = GameManager.Instance.InputManager.GetButtonBindingIconIndex(c.UI.Tab_Right.name, PlayerIndex);

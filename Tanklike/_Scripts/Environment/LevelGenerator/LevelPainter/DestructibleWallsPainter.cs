...
namespace TankLike.Environment.LevelGeneration
{
    ...
    public class DestructibleWallsPainter : RoomPainter
    {
        ...
        private const float DESTRUCTIBLE_WALL_PROBABILITY = 0.8f;
        ...
        public override void PaintRoom(MapTiles_SO map, Room room)
        {
            ...
            //Debug.Log($"probabilities are: random: {probabilities.x}, wall: {probabilities.y}, destructible wall: {probabilities.z}");
            //Debug.Log($"attempted to build {attempts.x}, succeeded {attempts.y}");
        ...
        private Tile GetTileNextToDestructibleWalls(List<Tile> groundTiles, List<Tile> tiles)
        {
            ...
            if (selectedTile == null)
            {
                Debug.Log("No tile next to destructible walls".Color(Colors.Red));
                selectedTile = tiles.RandomItem();
            }

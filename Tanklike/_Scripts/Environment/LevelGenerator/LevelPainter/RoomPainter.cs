...
namespace TankLike.Environment.LevelGeneration
{
    ...
    public abstract class RoomPainter : MonoBehaviour, IManager
    {
        ...
        {
            NeighbourWithinDepthByType = 0,
            NeighbourWithinDepthByTag = 1,
            NeighbourWithinRange = 2,
            IsOfType = 3
        }
                ...
                for (int i = 0; i < rules.Count; i++)
                {
                    ...
                    switch (paintingRule.FilterType)
                    {
                        case FilterType.IsOfType:
                            {
                                if (paintingRule.PerformOpposite)
                                {
                                    rulePassed = t.GetTileType() != paintingRule.TileType;
                                }
                                else
                                {
                                    rulePassed = t.GetTileType() == paintingRule.TileType;
                        ...
                        case FilterType.NeighbourWithinRange:
                            {
                                if (paintingRule.PerformOpposite)
                                {
                                    rulePassed = !HasNeighborAtExactDepth(t, paintingRule.TileType, paintingRule.MinDepth, paintingRule.MaxDepth, inputTiles);
                                }
                                else
                                {
                                    rulePassed = HasNeighborAtExactDepth(t, paintingRule.TileType, paintingRule.MinDepth, paintingRule.MaxDepth, inputTiles);
        ...
        protected bool HasNeighborAtExactDepth(Tile tile, TileType targetType, int exactDepthMin, int exactDepthMax, List<Tile> tiles)
        {
            if (exactDepthMin <= 0)
            {
                return false;

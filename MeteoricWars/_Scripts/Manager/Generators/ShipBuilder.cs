...
#region Main Ship Helper Functions
        ...
        {
            EnemyStats stats = m_LastCreatedShip.GetComponent<EnemyStats>();
            ...
            // multiplay health if there are two players
            float health = Mathf.Round(m_SelectedValues.HealthValue.Lerp(m_DefensiveDifficulty));

            // increase enemies health if there are two players
            if (GameManager.i != null)
            {
                if (GameManager.i.PlayersManager != null)
                {
                    health *= GameManager.i.PlayersManager.Players.Count == 2 ? 1.5f : 1f;
                }
            }

            components.Health.SetUpStats(health);

            stats.Rank = m_SelectedValues.Rank;
            stats.ScoreValue = m_SelectedValues.Score;

            //stats.Components = components;
            //components.ShipShooter = m_LastCreatedShip.GetComponent<Shooter>();

            stats.SetUserTag(BulletUser.Enemy);
            stats.SetDamage(m_SelectedValues.DamageRange.Lerp(m_OffensiveDifficulty));

            // selecting a random shooting method for the ship
            m_LastCreatedShip.AddComponent(m_ShootingTypes.RandomItem());

            m_LastCreatedShip.GetComponent<EnemyShooting>().Components = components;
            components.ShootingMethod = m_LastCreatedShip.GetComponent<EnemyShooting>();
            ...
            if (m_SelectedValues.Rank == ShipRank.Strong || m_SelectedValues.Rank == ShipRank.Medium
                || m_SelectedValues.Rank == ShipRank.Summoner)
            {
                m_LastCreatedShip.GetComponent<EnemyComponents>().Abilities = m_LastCreatedShip.AddComponent<EnemyShipAbilities>();
            }
        }
        ...
        private void SetUpColliderAndDamageDetection()
        {
            ...
            PolygonCollider2D collider = m_LastCreatedShip.AddComponent<PolygonCollider2D>();
        ...
        private void SetUpColoring(List<ShipPart> _parts)
        {
            ...
            if (m_ColorProfile == null) SetNewColorPalette();

...
namespace SpaceWar
{
    public class PlayerHealth : ShipHealth, IComponent, IController
    {
        ...
        public UnityAction<int, string> OnPlayerDeathAction;
        ...
        public void SetUp(ShipComponenets components)
        {
        ...
        public override void TakeDamage(float _amount, int _userIndex = -1)
        {
            // if the health recieved is a damage, then play the damage audio
            if (_amount < 0)
            {
                AudioManager.i.PlayAudio(GameManager.i.GeneralValues.HurtAudio);
            }
            ...
            if (CurrentHealth <= 0f)
            {
                ...
                {
                    OnPlayerDeathAction(m_Components.PlayerIndex, m_Components.gameObject.name);
                }

                m_Components.Stats.OnShipDestroyed(_userIndex);
                CanBeShot = false;
            }
        ...
        private IEnumerator RegeneratingProcess()
        {
            ...
            {
                TakeDamage(HEALTH_GENERATED_PER_SECOND);
